...
function getDst(len, output) {
    if (output === undefined)
        return new Uint8Array(len);
    (0, _assert_js_1.abytes)(output);
    if (output.length < len)
        throw new Error('aes: invalid destination length, expected at least ' + len + ', got: ' + output.length);
    if (!(0, utils_js_1.isAligned32)(output))
        throw new Error('destination must not be unaligned');
    return output;
}
